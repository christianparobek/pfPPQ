...
bottom_row_theme <- theme(axis.text = element_text(angle = 45, hjust = 1, size = rel(0.6)), axis.text.y = element_blank())
...
## PLOT PPQ COLUMN ##
p_cp1_ppq <- ggplot(cp1_ppq, aesthetics) + geometry + theme + cp1_color + top_row_theme + labs(title = "PPQR")
...
## PLOT MQ COLUMN ##
p_cp1_mq <- ggplot(cp1_mq, aesthetics) + geometry + theme + cp1_color + top_row_theme + labs(title = "MQR")
...
grid.arrange(p_cp1_art, p_cp1_k13, p_cp1_mq, p_cp1_ppq, p_cp1, 
             ...
             ncol = 5, heights=c(0.9,.588,1,1.25), widths=c(3.2,1,0.5,2,1))
grid.text("ART-R", x = unit(0.3, "npc"), y = unit(0.975, "npc"), gp=gpar(fontsize=10))
grid.segments(x0 = unit(0.09, "npc"), y0 = unit(0.96, "npc"),
              x1 = unit(0.525, "npc"), y1 = unit(0.96, "npc"))
